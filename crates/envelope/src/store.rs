...
const MAX_DECODED: usize = 2 * 1024 * 1024;
...
pub fn decompress_store_payload(body: &[u8]) -> Result<Vec<u8>, StoreError> {
    ...
    if is_gzip(body) {
        let mut decoder = GzDecoder::new(body).take(MAX_DECODED as u64 + 1);
        let mut out = Vec::new();
        decoder
            .read_to_end(&mut out)
            .map_err(|_| StoreError::DecompressFailed)?;
        if out.len() > MAX_DECODED {
            return Err(StoreError::DecompressFailed);
        }
        return Ok(out);
    }

    if is_zlib(body) {
        let mut decoder = ZlibDecoder::new(body).take(MAX_DECODED as u64 + 1);
        let mut out = Vec::new();
        decoder
            .read_to_end(&mut out)
            .map_err(|_| StoreError::DecompressFailed)?;
...
mod tests {
    ...
    #[test]
    fn rejects_oversize_gzip_payload() {
        let huge = vec![b'a'; MAX_DECODED + 8];
        let mut encoder = GzEncoder::new(Vec::new(), Compression::default());
        encoder.write_all(&huge).expect("gzip write");
        let compressed = encoder.finish().expect("gzip finish");
        assert!(matches!(
            decompress_store_payload(&compressed),
