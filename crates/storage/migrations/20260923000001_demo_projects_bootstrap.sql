-- Demo/preview projects for new workspaces + is_demo flag for the test banner.
-- Sample events include stack frames + breadcrumbs (same fixtures as scripts/seed).

ALTER TABLE projects
    ADD COLUMN IF NOT EXISTS is_demo BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN projects.is_demo IS
    'Preview/demo project with sample errors; skips setup wizard and shows test-data banner.';

-- Mark the well-known EPURE_DEV_SEED Acme projects when present.
UPDATE projects
SET is_demo = true
WHERE id IN (
