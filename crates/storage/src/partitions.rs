...
pub fn partition_bounds(year: i32, month: u32) -> (chrono::DateTime<Utc>, chrono::DateTime<Utc>) {
    let start = NaiveDate::from_ymd_opt(year, month, 1)
        .expect("valid partition month")
        .and_hms_opt(0, 0, 0)
    ...
    let end = NaiveDate::from_ymd_opt(next_year, next_month, 1)
        .expect("valid next partition month")
        .and_hms_opt(0, 0, 0)
        .expect("valid start of day")
        .and_utc();
    (start, end)
...
pub async fn ensure_partition(pool: &PgPool, year: i32, month: u32) -> Result<(), sqlx::Error> {
    ...
    let (start, end) = partition_bounds(year, month);
    let sql = format!(
...
pub async fn drop_expired_partitions(pool: &PgPool) -> Result<Vec<String>, sqlx::Error> {
    ...
    for relname in partitions {
        ...
        let (_start, partition_end) = partition_bounds(year, month);
        let retention_days =
            max_retention_for_partition(pool, _start, partition_end).await?;
