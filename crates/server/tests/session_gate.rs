...
async fn password_login_returns_org_scoped_issues_only() {
    ...
    support::seed_dev_pool(&pools.ingest)
        .await
        .expect("seed dev org");
    seed_dev_password(&pools.ingest).await.expect("seed dev password");
    seed_dev_issue(&pools.ingest).await.expect("seed dev issue");
    seed_org_b_user_and_issue(&pools.ingest)
        .await
        .expect("seed org b");
