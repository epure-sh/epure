...
async fn ttl_drops_old_partition_preserving_issue_counts() {
    ...
    assert!(partitions::partition_exists(&pools.ingest, &partition_name)
        .await
    ...
    let dropped = run_ttl_once(&migrate_pool).await.expect("run ttl once");
    assert!(
    ...
    let event_count = issue_event_count(&migrate_pool, issue_id)
        .await
