...
async fn setup_progress_patch_advances_steps() {
    ...
    let test = client
        .post(format!(
            "{base_url}/api/v1/setup/test-event?project_id={pid}"
        ))
        .send()
        .await
        .expect("test event");
...
#[tokio::test]
async fn seeded_project_setup_skips_onboarding() {
    let database_url =
        std::env::var("DATABASE_URL").expect("DATABASE_URL must be set for integration tests");
    let migrate_pool = connect(&database_url).await.expect("connect migrate pool");
    run_migrations(&migrate_pool).await.expect("run migrations");
...
async fn setup_test_event_creates_issue() {
    ...
    for _ in 0..20 {
        ...
        let issues = client
            .get(format!(
                "{base_url}/api/v1/issues?project_id={pid}&q=is:unresolved"
            ))
            .send()
            .await
            .expect("issues");
