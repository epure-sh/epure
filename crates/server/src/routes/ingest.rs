...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/api/{project_id}/user-feedback/",
            options(cors_preflight),
        )
        .route(
            "/api/{project_id}/user-feedback/",
            post(post_user_feedback),
        )
...
async fn post_envelope(
    ...
    let auth = match extract_auth(&headers, &query) {
        Ok(auth) => auth,
        Err(_) => return error_response(StatusCode::UNAUTHORIZED, "invalid_dsn", "DSN authentication failed"),
    };

    if let Err(response) = validate_dsn(&state, &auth, project_id).await {
        return response;
    }
...
async fn post_release_files(
    ...
    let release_id = match releases::ensure_release(&state.pools.ingest, project_id, &version).await {
        Ok(id) => id,
        Err(_) => {
            return error_response(
    ...
    let storage_path = match resolve_artifact_path(
        &state.artifacts_dir,
        project_id,
        &version,
        &file_name,
    ...
    let storage_path = match canonical_artifact_path(&storage_path) {
        Ok(path) => path,
        Err(_) => {
            return error_response(
                StatusCode::BAD_REQUEST,
...
async fn post_store(
    ...
    let payload = match parse_store_payload(&body) {
        Ok(payload) => payload,
        Err(StoreError::Empty) | Err(StoreError::InvalidJson) | Err(StoreError::DecompressFailed) => {
            return error_response(
                StatusCode::BAD_REQUEST,
                "invalid_store",
...
async fn post_user_feedback(
    ...
    let exists = match user_feedback::event_exists(&state.pools.ingest, project_id, body.event_id).await {
        Ok(value) => value,
        Err(_) => {
            return error_response(
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn accept_payload(
    ...
    with_cors(
        ...
        (
            StatusCode::ACCEPTED,
            Json(IngestAccepted { id: event_id }),
        )
            .into_response(),
    )
}
...
fn extract_auth(
    ...
    let header = headers
        .get("X-Sentry-Auth")
        .and_then(|value| value.to_str().ok());
    parse_auth(header, query.sentry_key.as_deref(), query.sentry_secret.as_deref())
}
...
fn cors_response(state: &AppState, headers: &HeaderMap, status: StatusCode) -> Response {
    let mut response = Response::builder().status(status).body(axum::body::Body::empty()).unwrap();
...
fn canonical_artifact_path(path: &std::path::Path) -> Result<std::path::PathBuf, ()> {
    let canonical = std::fs::canonicalize(path.parent().ok_or(())?).map_err(|_| ())?;
...
fn apply_cors_headers(state: &AppState, request_headers: &HeaderMap, response_headers: &mut HeaderMap) {
