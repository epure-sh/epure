...
fn validate_events(events: &[String]) -> bool {
    !events.is_empty()
        && events.iter().all(|event| {
            matches!(
                event.as_str(),
                "issue_created" | "regression" | "velocity_spike" | "custom_rule"
            )
        })
}
...
async fn update_webhook(
    ...
    if !validate_events(&body.events) {
        return Err(StatusCode::BAD_REQUEST);
    }

    let row = webhooks::update_events(&state.pools.app, dashboard.org_id, id, body.events)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
