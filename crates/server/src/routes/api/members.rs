...
async fn list_invitations(
    ...
    let rows = members::list_invitations(&state.pools.app, dashboard.org_id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn update_member(
    ...
    let updated =
        members::update_member_role(&state.pools.app, dashboard.org_id, user_id, &body.role)
            .await
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn cancel_invitation(
    ...
    let deleted = members::delete_invitation(&state.pools.app, dashboard.org_id, id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
