...
const TEST_ENVELOPE: &[u8] =
    include_bytes!("../../../../../fixtures/sentry/browser/envelope.txt");
...
async fn to_response(
    ...
    let has_active_key = dsn_keys::first_active_public_key(&state.pools.app, org_id, row.project_id)
        .await
...
async fn get_setup_dsn(
    ...
    let public_key = dsn_keys::first_active_public_key(
        &state.pools.app,
        dashboard.org_id,
        query.project_id,
    )
