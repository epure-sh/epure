...
use epure_auth::{
    ...
    CredentialError, GoogleAuth, GoogleAuthError,
};
...
async fn google_start(
    ...
    session
        ...
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    if let Some(invite_token) = query.invite.as_deref().map(str::trim).filter(|v| !v.is_empty()) {
        session
    ...
    session.save().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
fn map_credential_error(error: CredentialError) -> StatusCode {
    match error {
        ...
        CredentialError::EmailMismatch | CredentialError::InvalidCredentials => StatusCode::UNAUTHORIZED,
        CredentialError::TransferOwnershipRequired => StatusCode::CONFLICT,
        CredentialError::Password(_) | CredentialError::Sqlx(_) => StatusCode::INTERNAL_SERVER_ERROR,
    }
}

fn map_google_error(error: GoogleAuthError) -> StatusCode {
    match error {
        GoogleAuthError::InvalidState | GoogleAuthError::TokenExchange | GoogleAuthError::UserInfo => {
