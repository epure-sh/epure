...
fn validate_status(status: &str) -> bool {
    matches!(status, "unresolved" | "resolved" | "ignored" | "regression")
}
...
async fn issue_trends(
    ...
    if body.issue_ids.len() > epure_storage::issues::MAX_BULK_IDS {
        return Err(StatusCode::BAD_REQUEST);
    }

    let trends = events::list_issue_trends(&state.pools.app, dashboard.org_id, &body.issue_ids)
        .await
...
async fn list_issues(
    ...
    filter.limit = Some(issues::MAX_ISSUE_LIST);
...
async fn bulk_update(
    ...
    if body.ids.len() > issues::MAX_BULK_IDS {
        return Err(StatusCode::BAD_REQUEST);
    }

    let updated = issues::bulk_update_status(&state.pools.app, dashboard.org_id, &body.ids, status)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
