...
Docker Compose v2. `docker compose up` pulls `ghcr.io/epure-sh/epure`. Optional: `cp .env.example .env` and change `EPURE_PORT` / `EPURE_PUBLIC_URL` if 8080 is taken.
...
Open [http://localhost:8080](http://localhost:8080) → register → name a project → copy the DSN → point your SDK. Ports and CORS: [Configuration](https://epure.sh/docs/self-hosting/configuration). Production: copy `.env.production.example`, HTTPS in front, [Installation](https://epure.sh/docs/self-hosting/installation).
