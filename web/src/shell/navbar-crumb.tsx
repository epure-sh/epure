...
export interface NavCrumbProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  label: string;
  badge?: ReactNode;
  /** When false, the label stays fully visible (environment names). */
  truncate?: boolean;
}

export const NavCrumb = forwardRef<HTMLButtonElement, NavCrumbProps>(
  ({ icon: Icon, label, badge, truncate = true, className, ...props }, ref) => (
    <button
      ref={ref}
      type="button"
      className={cn(
        "epure-navbar-crumb",
        navControl,
        "inline-flex h-8 items-center gap-1 px-1.5 text-xs tracking-ui text-ink hover:bg-bg-subtle/60",
        truncate ? "max-w-[10rem]" : "max-w-none shrink-0",
        className,
      )}
      {...props}
    >
