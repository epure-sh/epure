      ...
      {items.map((item) => {
        ...
        if (!item.onClick) {
          return (
            <div
              key={item.label}
              className="epure-stat-card rounded-lg border border-border bg-surface px-4 py-3 text-center"
            >
              {content}
            </div>
          );
        ...
        return (
          <button
            ...
            className={cn(
              "epure-stat-card rounded-lg border bg-surface px-4 py-3 text-center transition-colors duration-fast focus-ring",
              item.active
...
export function StatBar({ items, className, inline = false }: StatBarProps) {
  ...
  if (variant === "cards" && !inline) {
    return (
      <div className={cn("epure-stat-bar border-b border-border bg-bg px-4 py-3", className)}>
