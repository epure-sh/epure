...
export function FilterChip({
  ...
  if (onRemove) {
    return (
      <span
        className={cn(
          "epure-filter-chip epure-filter-chip--query group/chip inline-flex max-w-full items-stretch overflow-hidden rounded-md border text-xs",
          variant === "query"
            ? "border-accent/30 bg-accent-muted/60 text-accent"
            : active
              ? "border-border-strong bg-state-selected text-ink"
  ...
  return (
    <button
      type="button"
      className={cn(
        "epure-filter-chip inline-flex h-control-sm max-w-full items-center gap-1 rounded-md border px-2.5 text-xs font-medium transition-colors duration-fast focus-ring",
        active
          ? "border-border-strong bg-state-selected text-ink"
