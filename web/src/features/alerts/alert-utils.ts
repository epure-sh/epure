...
export type AlertKindFilter =
  ...
  | "users_affected"
  | "ingest_cap_hit";
export type AlertViewMode = "attention" | "activity" | "all";
export type AlertSort = "newest" | "oldest" | "severity";
export type AlertTimeWindow = "24h" | "7d" | "30d" | "all";
export type AlertSeverity = "warning" | "danger";

const ATTENTION_KINDS = new Set(["velocity_spike", "velocity", "regression", "ingest_cap_hit"]);
const ACTIVITY_KINDS = new Set(["new_issue", "event_milestone", "users_affected"]);
...
export const ALERT_VIEW_MODES: {
...
}[] = [
  {
    ...
    description: "Velocity spikes, regressions, and ingest caps",
  },
  {
    value: "activity",
    label: "Activity",
    description: "New issues, volume milestones, and user impact",
  },
  {
    value: "all",
    label: "All",
...
export function countAlertsForViewModes(
  ...
  return {
    attention: filterAlertsByViewMode(scoped, "attention").length,
    activity: filterAlertsByViewMode(scoped, "activity").length,
    all: scoped.length,
  };
}
...
export function loadAlertsPreferences(): AlertsPreferences {
  try {
    ...
    const viewMode =
      ...
      (parsed.defaultKindFilter === "all" ? "all" : "attention");
    return {
