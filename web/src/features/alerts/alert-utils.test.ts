...
assert.equal(filterAlertsByViewMode([velocity, regression], "attention").length, 2);
assert.equal(filterAlertsByViewMode([velocity, regression], "activity").length, 0);
assert.equal(countAlertsForViewModes([velocity, regression], "7d").attention, 2);
