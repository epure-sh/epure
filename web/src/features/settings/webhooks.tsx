...
function eventLabel(event: string): string {
  return WEBHOOK_EVENT_OPTIONS.find((item) => item.id === event)?.label ?? event;
}
...
export function WebhooksSettings() {
  ...
  async function handleToggleExistingEvent(hook: WebhookRow, eventId: WebhookEventId, checked: boolean) {
    const next = checked
      ? hook.events.includes(eventId)
        ? hook.events
        : [...hook.events, eventId]
      : hook.events.filter((value) => value !== eventId);
    if (next.length === 0) {
    ...
    setBusy(true);
    try {
      const updated = await updateWebhook(hook.id, { events: next });
      setWebhooks((rows) => rows.map((row) => (row.id === hook.id ? updated : row)));
    } catch {
      toast("Failed to update events");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <InfoCallout>
        ...
        Velocity spikes, volume milestones, and custom rules can be forwarded if you subscribe
        below. The{" "}
        <Link
      ...
      <Card>
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={(event) => void handleCreate(event)}>
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-ink">Payload preview</p>
              <p className="text-xs text-ink-muted">
                Sample {formatLabel(format)} body for{" "}
                {eventLabel(events[0] ?? "issue_created")}. Fields stay the same across events.
              </p>
              <pre className="overflow-x-auto rounded-md border border-border bg-bg-subtle p-3 font-mono text-xs text-ink">
                {previewWebhookPayload(format, events[0] ?? "issue_created")}
              </pre>
            </div>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 space-y-1.5">
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {WEBHOOK_EVENT_OPTIONS.map((item) => (
                          <label key={item.id} className="flex items-center gap-1.5 text-xs text-ink-muted">
                            <Checkbox
                              checked={hook.events.includes(item.id)}
                              disabled={busy}
                              onCheckedChange={(checked) =>
                                void handleToggleExistingEvent(hook, item.id, checked === true)
                              }
                              aria-label={`${item.label} for endpoint`}
                            />
                            {item.label}
                          </label>
                        ))}
