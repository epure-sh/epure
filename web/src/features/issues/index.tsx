...
export function IssuesPage() {
  ...
  useEffect(() => {
    if (!projectId || !setup || setup.complete || !setup.dsn_copied_at) {
      return;
    }
    if (issues.length === 0) {
      return;
    ...
    void fetchSetupProgress(projectId)
      .then((progress) => {
        setSetup(progress);
        if (progress.complete && !celebratedFirstIssue.current) {
  ...
  }, [issues.length, projectId, setup?.complete, setup?.dsn_copied_at, toast]);
  ...
  const needsSetupRedirect =
    ...
    setup &&
    !setup.complete &&
    (!setup.project_named || !setup.has_active_key || !setup.dsn_copied_at);
      ...
      <div className="flex h-full min-h-0 flex-col">
        {setup ? (
          <SetupChecklist
            ...
            hasActiveKey={setup.has_active_key}
